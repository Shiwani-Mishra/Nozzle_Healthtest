...
def compute_rmse(data, cone_apex, cone_direction):
    distances = []
    for point in data:
        # Vector from cone apex to data point
        v = point - cone_apex
        # Project v onto cone direction
        projection = np.dot(v, cone_direction) / np.linalg.norm(cone_direction)
        # Subtract the projection from v to get the perpendicular distance
        perpendicular_distance = np.linalg.norm(v - projection * cone_direction)
...
if __name__ == "__main__":
    ...
    # After fitting the cone
    cone_apex, cone_direction = fit_cone(noisy_data)
    rmse = compute_rmse(noisy_data, cone_apex, cone_direction)
